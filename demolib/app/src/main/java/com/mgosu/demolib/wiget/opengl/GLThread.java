package com.mgosu.demolib.wiget.opengl;
...
class GLThread extends Thread {
	...
	private int mWidth;
	private int mHeight;
	private int mRenderMode = GLWallpaperService.GLEngine.RENDERMODE_CONTINUOUSLY;
	...
	public void setRenderMode(final int pRenderMode) {
		if (GLWallpaperService.GLEngine.RENDERMODE_WHEN_DIRTY != pRenderMode && pRenderMode != GLWallpaperService.GLEngine.RENDERMODE_CONTINUOUSLY) {
			throw new IllegalArgumentException("Illegal pRenderMode: '" + pRenderMode + "'.");
		}
		synchronized (this.sGLThreadManager) {
			this.mRenderMode = pRenderMode;
			if (pRenderMode == GLWallpaperService.GLEngine.RENDERMODE_CONTINUOUSLY) {
			...
			while (!this.isDone()) {
				...
				synchronized (this.sGLThreadManager) {
					while (true) {
						...
						if ((!this.mPaused) && this.mHasSurface && this.mHaveEGL && (this.mWidth > 0) && (this.mHeight > 0) && (this.mRequestRender || (this.mRenderMode == GLWallpaperService.GLEngine.RENDERMODE_CONTINUOUSLY))) {
